{ config, pkgs, lib, ... }:

{
  # Enable SSH on boot
  services.openssh = {
    enable = true;
    settings.PermitRootLogin = "yes";
  };

  # Add SSH keys for both nixos and root users
  users.users.nixos = {
    openssh.authorizedKeys.keys = [
      "ssh-rsa  bobby@bob-mac.local"
    ];
  };
  users.users.root.openssh.authorizedKeys.keys = config.users.users.nixos.openssh.authorizedKeys.keys;

  # Enable mDNS for .local discovery
  services.avahi = {
    enable = true;
    nssmdns4 = true;
    publish = {
      enable = true;
      addresses = true;
      workstation = true;
    };
  };

  # Basic networking
  networking = {
    hostName = "nixos-installer";
    useDHCP = true;
    firewall.enable = false;
  };

  # Helpful packages for installation
  environment.systemPackages = with pkgs; [
    vim
    git
    tmux
    htop
    dig
    curl
    wget
  ];

  # Show IP address on console
  services.getty.helpLine = ''

    The system is accessible via SSH. Your IP addresses are:
    IPv4: $(ip -4 addr show scope global | grep inet | awk '{print $2}' | cut -d/ -f1 | paste -sd ' ')

    Default user: nixos (passwordless sudo)

  '';

  # Print network info periodically
  systemd.services.show-ip = {
    description = "Show IP addresses on console";
    after = [ "network-online.target" ];
    wants = [ "network-online.target" ];
    serviceConfig = {
      Type = "oneshot";
      ExecStart = "${pkgs.bash}/bin/bash -c 'echo Network addresses: && ip -4 addr show scope global && ip -6 addr show scope global'";
      StandardOutput = "journal+console";
    };
  };

  systemd.timers.show-ip = {
    wantedBy = [ "timers.target" ];
    timerConfig = {
      OnBootSec = "30s";
      OnUnitActiveSec = "5min";
    };
  };
}
